...
void check_yasp(bool p0 = false) {
    ...
    // Test the interface
    Opm::GridInterfaceEuler<Dune::YaspGrid<dim> > gie(grid);
#if HAVE_MPI
    //    if (rank == 0) {
...
void check_sgrid()
{
    ...
    std::cout << std::endl << "Dune::SGrid, dim = " << dim << ", refinement = " << refinement;
    std::cout << std::endl << std::endl;
    ...
    // Test the interface
    Opm::GridInterfaceEuler<Dune::SGrid<dim, dim, double> > gie(grid);
    test_interface(gie);
}
...
//-----------------------------------------------------------------------------
{
    ...
    // Test the interface
    Opm::GridInterfaceEuler<Dune::CpGrid> gie(grid);
    test_interface(gie);
}
